 ...
 * @Author: WQY
 * @CreateTime: 2024-03-29  14:27
 * @Description: TODO
 * @Version: 12.1.0
 */
public class XuguSqlAstTranslator<T extends JdbcOperation> extends SqlAstTranslatorWithUpsert<T> {
    ...
    @Override
    protected AbstractSqlAstTranslator.LockStrategy determineLockingStrategy(
            QuerySpec querySpec,
            AbstractSqlAstTranslator.ForUpdateClause forUpdateClause,
        ...
        if ( strategy != AbstractSqlAstTranslator.LockStrategy.FOLLOW_ON && isPartOfQueryGroup() ) {
        ...
        if ( strategy != AbstractSqlAstTranslator.LockStrategy.FOLLOW_ON && hasSetOperations( querySpec ) ) {
            if ( followOnLockingDisabled ) {
                throw new IllegalQueryOperationException( "Locking with set operators is not supported" );
            }
            strategy = AbstractSqlAstTranslator.LockStrategy.FOLLOW_ON;
        }
        if ( strategy != AbstractSqlAstTranslator.LockStrategy.FOLLOW_ON && needsLockingWrapper( querySpec ) && !canApplyLockingWrapper( querySpec ) ) {
            if ( followOnLockingDisabled ) {
    ...
    private boolean isPartOfQueryGroup() {
        return getQueryPartStack().findCurrentFirst( XuguSqlAstTranslator::partIsQueryGroup ) != null;
    }
